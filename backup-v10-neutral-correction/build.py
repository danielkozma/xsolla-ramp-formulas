...
# is asymmetric, like the lightness lobe: it runs to hue 60 on the green side and
# dies at 342 on the warm side, so it covers the orange band alone and leaves
# Pulse and Pink on the whole drop.
#
...
S_UI_LOBE_UP_V7 = 28.0   # 32° → 60° (orange into yellow) — gone by 60
...
def S_hue_gain_v7(H):
    ...
    asymmetric for the same reason: it reaches further on the green side."""
...
# Adjusted correction (brand colours only). The lobe above only ever darkens, so
# the blues it leaves alone read far darker than everything else — worst in the
# mids and darks, where hue 250 sits 10–14 OKLab points under grey. The adjusted
# weight is one smooth curve round the whole wheel instead of a lobe. It passes
# through a short list of (hue, weight) points — each one a peak or a trough —
# and every stretch between two neighbours is a half-cosine:
#
#     w(H) = w₁ + (w₀ − w₁)·(1 + cos(π·x))/2,   x = (H − H₀)/(H₁ − H₀)
#
# A half-cosine is flat at both ends, so the joins have no corners. The curve
...
def L_v7(t, H=None, adjusted=False, curve=BRAND_CURVE_V7):
    if t <= 0.0:
        return 100.0
    dark = t ** L_exponent_v7(t, H, adjusted, curve)
    if adjusted and H is not None:
        w = hue_weight_v7(H, True, curve)
        if w < 0.0:
            dark *= 1.0 + curve[1] * w * lift_shape_v7(t)   # adjusted only: lift the darks
...
def gradient_css_v7(kind, hue, theme, n=40, semantic=False, adjusted=False):
    stops = [f"{sample_hex_v7(kind, hue, theme, i/(n-1), semantic, adjusted)} {round(100*i/(n-1), 2)}%"
             for i in range(n)]
    return f"linear-gradient(to bottom, {', '.join(stops)})"
...
def build_family_v7(name, kind, theme, hue, step_keys, orig_map, v5_rows, group="",
    ...
    lift = round(-k[1] * w, 3) if w < 0.0 else None   # λ·|w|, the multiplier on u(t)
    v5_by_step = {r["step"]: r["hsl"] for r in v5_rows}
    d5 = []
    for key in step_keys:
        t = step_to_t_v5(key)
    ...
    return {
        ...
        "dip": dip, "lift": lift, "rows": rows,
        "d5": round(sum(d5)/len(d5), 2) if d5 else None,
        "gradient": gradient_css_v7(kind, hue, theme, semantic=sem, adjusted=adjusted),
        "positions": [round(t, 4) for t in positions],
    }
...
def build_v7(v5):
    ...
    out = {
        ...
        "meta": {
            ...
            "S_g": "g(H) = 1 \u2212 0.4\u00b7cos\u00b2(\u00bd\u03c0\u00b7d), d = |H\u221232| / (28 up, 50 down)",
    ...
    for group, fams in GROUPS_V7:
        for fam in fams:
            ...
            if group == "Brand & Labels":
                out["families_adjusted"].append(build_family_v7(
                    fam, "chromatic", "both", HUES[fam], keys, P["light"][fam],
                    v5_fam[(fam, "both")], group, adjusted=True))
