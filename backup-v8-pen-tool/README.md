...
## Pen tool — the same model, drawn instead of written

The app bar carries a two-way switch: **Math** is the page above, **Pen tool**
is the same model with its six curves exposed in a Bézier editor. The mast, the
ΔE legend and the family cards stay put across the switch; the formula panel,
the step ladder and the playgrounds belong to Math, the editors to Pen. The
choice is kept in `localStorage`, and Math is the default.

Nothing about the colour model changes. Lightness is still

```
e(t,H) = base(t) − 0.20·w(H)·b(t)
L(t,H) = 100 − 100·t^e(t,H)
```

### Anchors and handles

A real pen tool, not a spline through points: each curve is a short chain of
anchors, every segment between two anchors is one cubic Bézier, and the two
handles that shape it are what you drag. Round anchors, square handles on thin
leaders.

| | curve | anchors | handles | seeded from |
|---|---|---|---|---|
| 1 | base exponent | 2 | 2 | flat at `P_BASE_V7` 0.84 |
| 2 | correction bell `b(t)` | 3 | 4 | `4c·t/(t+c)²`, `c = DIP_C_V7` 0.09 — **ends locked at y = 0**, peak in the middle |
| 3 | hue strength `w(H)` | 3 | 6 | cos² lobe, peak 75°, 145 up / 90 down |
| 4 | shared Hill | 2 | 2 | `100 − 30·(3t)⁶/(1+(3t)⁶)` |
| 5 | UI Hill (Pulse · Flash · Pink) | 2 | 2 | `100 − 106·u/(1+u)`, `u = (t/0.625)³`, before the hue gain |
| 6 | UI hue gain `g(H)` | 3 | 6 | cos² lobe, centre 32°, 28 up / 50 down, cut 0.4 |

Handle **x** is stored as a fraction of its own segment's x-span, so x can never
fold back on itself and `y(x)` stays a function however hard a handle is pulled.
Handle **y** is free — that is where the bell gets its shape. Evaluation solves
`x → s` by Newton inside a bisection bracket, the way a browser evaluates a
`cubic-bezier` easing.

Interior anchors are **smooth**: drag one handle and its pair turns to stay
collinear, keeping its own length. The mirror is computed in plot pixels, since
an angle means nothing while one axis is degrees and the other is 0–1.
**Alt-drag breaks** a smooth anchor. Each plot's y range is deliberately wider
than its curve so the handles have somewhere to go — the shared Hill's out
handle sits at y ≈ 120, above the 100 line, which is how one segment holds a
long flat run.

The two hue curves **wrap**. Their anchors sit on the shape's own features
(lobe peak, lobe end, lobe start) rather than at 0° and 360°, so the seam is
just another segment; the chain is drawn twice, a turn apart and clipped to the
plot, and a handle that lands past 360° is shown a turn back so it stays
grabbable where its hue actually is.

### Split view

On a screen 1000px or wider, Pen mode lays the page out as two panes: **palettes
on the left, curves on the right**, each with its own scrollbar, so a ramp stays
in view while its curve is being pulled. Drag the divider between them to
resize — 25% to 75%, double-click or `Home` to recentre at 52%, arrow keys to
nudge (shift for fine). The width is kept in `localStorage` under `xcf.split`.

The markup is not duplicated: entering the split moves every child of `.wrap`
into a pane and leaving moves them all back in their recorded order, so Math
mode gets exactly the document it always had. Below 1000px Pen mode falls back
to the ordinary stacked page.

The app bar is still static, never fixed or sticky — in the split the document
is exactly one viewport tall, so there is nothing for it to scroll away from.
The button row carrying **Reset all**, the two **Copy** buttons and the live ΔE
was lifted out of the pen header into its own row so it can stick to the top of
the right pane: the ΔE is what you watch while pulling a handle, and it has to
stay on screen when you scroll down to the sixth curve.

### The x-axis

**Log t on every `t` curve**, from step 20 to step 1000. The tokens are a
doubling scale, so 25 / 50 / 100 / 200 / 400 / 800 land evenly across the plot
and are labelled on the axis; 300 / 500 / 600 / 700 / 850 / 900 get minor ticks.
On a linear axis the four lightest tokens share the first tenth of the width and
there is nothing to drag.

### What the seed costs

The handles open fitted to the v7 functions — a least-max fit of each chain over
the token range, so the pen page opens as close to Math as this many anchors
reach. It is **not** an identity, and the page says so rather than hiding it:
the header quotes the live max ΔE against Math with the seed figure beside it.

| | max ΔE vs Math, seeded |
|---|---|
| Neutrals (light and dark) | **0.000** |
| Flash · Pink · Edge · Yellow | 0.44 – 0.84 |
| Brand · Mindaro · Core · Mint | 0.87 – 1.16 |
| Pulse | **1.40** |
| mean over all 132 swatches | **0.27** |

Where that comes from, curve by curve: the two **cos² lobes are essentially
exact** — max error 0.0005 on `w(H)` and 0.0002 on `g(H)`. Every lobe half fits
on a handle fraction of **≈0.3628**, the standard cubic-Bézier approximation to
a quarter cosine, which is exactly what a cos² lobe half is. The **bell** is
within 0.12 across the token range. The error is all in the **Hills**: 3.61
points of S on the shared one, 2.71 on the UI one, because the palette turns
twice between step 250 and step 550 and a single cubic segment turns once. Three
anchors on each Hill would bring the whole palette to 0.28 and four to 0.31 —
the anchor counts here are the ones asked for, and this is their price.

The bell's locked left end costs nothing at any token, since no token lives
between step 20 and step 25; the dashed Math reference on that plot is where it
shows.

### Live checks

Every card quotes mean and max ΔE against the Figma originals and max ΔE against
v7, recomputed on each drag. The closed form guaranteed a strictly darkening
ramp by construction; a dragged curve does not, so each ramp is checked step by
step in OKLab L and any step that fails is ringed in red on the card, called out
in the card header, and counted in the pen header. In practice these anchor
counts make it hard to break — two anchors cannot put a local jump into a Hill —
and the seeded palette is clean, but the check runs on every drag regardless.

### Getting work out

**Reset to Math** is on every curve and on all six at once. Dragged curves
persist across a reload (`localStorage`, key `xcf.pen.v2`) — a shape worth ten
minutes should not be one accidental ⌘R from gone, and Reset is the undo. A
saved shape whose anchor count no longer matches the seed is dropped rather than
patched. **Copy control points** exports the six chains as JSON — anchors with
their token positions, both handles per segment in absolute and raw form, plus
the axis definitions; **Copy ramps** exports the resulting hex for every family
and step.

All of it runs in the browser. `build.py`, `data/generated.json` and the
`page.py` payload are untouched.
...
| | |
|---|---|
| `index.html` | built page — v7 only, cards grouped Brand & Labels · UI (Semantic) · Neutrals, a playground under each, Math / Pen tool switch in the app bar |
